...
def save_image(image_dicts, input_image_name, output_dir='./results'):
...
def gen_cam(image, mask, cam_info=None):
    ...
    if cam_info is not None:
        ...
        fshape = 'feature shape: ' + str(cam_info[1])
        # cv2.putText(影像, 文字, 座標, 字型, 大小, 顏色, 線條寬度, 線條種類)
        cv2.putText(heatmap, layer, (x, y), font, 1.5, fcolor, lw, cv2.LINE_AA)
        cv2.putText(heatmap, fshape, (x, y2), font, 1.5, fcolor, lw, cv2.LINE_AA)
...
def main(index):
    ...
    model = init_detector(cfg, ckpoint)
    model.to('cpu')
    img = '/home/r07631006/data/VOCdevkit/VOC2007/JPEGImages/1295.jpg'
    ...
    cvimg = original_image[..., ::-1]
    # x1, y1, x2, y2 = box
    image_dict['predict_box'] = cvimg
    image_cam, image_dict[layer_name] = gen_cam(cvimg, mask, caminfo)
